...
reg       pair_is_flush;
reg [5:0] pair_out_r;
reg [5:0] pair_out_c_base;
reg [6:0] pair_center_r;

// Front-end pipeline before cube-root.
reg        mac_valid;
reg signed [19:0] mac_prod0 [0:8], mac_prod1 [0:8];
reg [11:0] mac_addr0, mac_addr1;

reg        sum_valid;
reg signed [19:0] sum_a0, sum_b0, sum_c0;
reg signed [19:0] sum_a1, sum_b1, sum_c1;
reg [11:0] sum_addr0, sum_addr1;

reg        clamp_valid;
reg [7:0]  clamp0, clamp1;
reg [11:0] clamp_addr0, clamp_addr1;

reg        square_valid;
reg [15:0] square_target0, square_target1;
reg [11:0] square_addr0, square_addr1;

// 2-lane, 6-stage cube-root pipeline. Each group carries 2 output pixels.
reg        pipe_valid [0:5];
reg [15:0] pipe_target0 [0:5], pipe_target1 [0:5];
reg [5:0]  pipe_lo0 [0:5], pipe_lo1 [0:5];
reg [5:0]  pipe_hi0 [0:5], pipe_hi1 [0:5];
reg [11:0] pipe_addr0 [0:5], pipe_addr1 [0:5];
...
// ============================================================
// Push one pair from a 4-pixel group into the MAC product stage.
...
task push_pair;
    ...
    input [6:0] center_r;
    input       pair_sel;
    reg [5:0] lane0_out_c, lane1_out_c;
    reg [6:0] c0, c1;
    begin
        lane0_out_c = out_c_base + (pair_sel ? 6'd2 : 6'd0);
        lane1_out_c = out_c_base + (pair_sel ? 6'd3 : 6'd1);
        ...
        mac_addr0 <= make_addr(out_r, lane0_out_c);
        mac_addr1 <= make_addr(out_r, lane1_out_c);
    end
endtask
        ...
        case (state)
            ...
            S_STREAM: begin
                if (pair_pending) begin
                    o_in_ready <= 1'b1;
                    push_pair(pair_out_r, pair_out_c_base, pair_center_r, 1'b1);
                    pair_pending <= 1'b0;
                end else begin
                    o_in_ready <= 1'b1;
                end

                if (!pair_pending && i_in_valid) begin
                    write_input_group();
                    ...
                    if (!stride_reg) begin
                        ...
                        if (read_row >= 7'd2) begin
                            ...
                            pair_pending <= 1'b1;
                            pair_is_flush <= 1'b0;
                            pair_out_r <= row_minus_2_low6(read_row);
                            pair_out_c_base <= {read_grp, 2'b00};
                            pair_center_r <= row_minus_2(read_row);
                        end
                    end else begin
                        ...
                        if ((read_row >= 7'd2) && (read_row[0] == 1'b0) && (read_grp <= 4'd7)) begin
                            ...
                            pair_pending <= 1'b1;
                            pair_is_flush <= 1'b0;
                            pair_out_r <= row_minus_2_div2(read_row);
                            pair_out_c_base <= {1'b0, read_grp[2:0], 2'b00};
                            pair_center_r <= row_minus_2(read_row);
                        end
                    end
            ...
            S_FLUSH: begin
                ...
                if (pair_pending) begin
                    push_pair(pair_out_r, pair_out_c_base, pair_center_r, 1'b1);
                    pair_pending <= 1'b0;
                    if (!pair_is_flush) begin
                        pair_is_flush <= 1'b0;
                    end else if (!stride_reg) begin
                    ...
                    pair_pending <= 1'b1;
                    pair_is_flush <= 1'b1;
                    pair_out_r <= flush_out_r;
                    pair_out_c_base <= {flush_grp, 2'b00};
                    pair_center_r <= {1'b0, flush_out_r};
                end else begin
                    ...
                    pair_is_flush <= 1'b1;
                    pair_out_r <= 6'd31;
                    pair_out_c_base <= {1'b0, flush_grp[2:0], 2'b00};
                    pair_center_r <= 7'd62;
                end
            end
